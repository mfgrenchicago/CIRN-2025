...
protocol ManufacturersAPIService {
	func fetchManufacturers(query: String) async throws -> [APICompany]
}
...
struct APIKey {
	static var key: String {
		let apiKey = Bundle.main.infoDictionary?["Some_Text"] as? String ?? ""
		return apiKey
	}
}
...
class ManufacturersAPIServiceImpl: ManufacturersAPIService {
	...
	func fetchManufacturers(query: String) async throws -> [APICompany] {
		...
		let requestBody = ["textQuery": query, "pageSize": "30"]
